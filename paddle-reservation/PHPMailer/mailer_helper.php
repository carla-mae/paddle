 ...
 * instead of raw SMTP. Many free hosts (Render, InfinityFree, etc.) block
 * outbound SMTP ports (25/465/587) at the network level, so plain PHPMailer
 * SMTP sending never gets a chance to even reach Gmail. This sends over
 * HTTPS instead, which works regardless of which host you deploy to.
 *
...
function send_smtp_mail(string $toEmail, string $toName, string $subject, string $bodyText): bool {
    ...
    if ($apiKey === '' || $fromAddress === '') {
        $errorMsg = 'Missing BREVO_API_KEY or MAIL_FROM_ADDRESS. Set these in your real .env file (not .env.sample), or in PHPMailer/mail_config.php.';
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        error_log("Email sent via Brevo API to {$toEmail}");
        file_put_contents(
            __DIR__ . '/mail_error.log',
